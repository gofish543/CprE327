...
int monster_event(Event* event) {
    ...
    if (monster->isAlive) {
        monster_move(monster, monster_is_tunneler(monster->classification) ? monster->character->floor->tunnelerView : monster->character->floor->nonTunnelerView);
    }
...
int monsters_battle(Monster* monsterA, Monster* monsterB) {
    if (monsterA->level > monsterB->level) {
        return 1;
    } else if (monsterB->level < monsterB->level) {
        return -1;
