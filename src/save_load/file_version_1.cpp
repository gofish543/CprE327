...
int file_save_floor_1(FILE* file, Floor* floor) {
    ...
    for (index = 0; index < floor->getStairUpCount(); index++) {
        if (floor->getUpStairs().size() < index) {
    ...
    for (index = 0; index < floor->getStairDownCount(); index++) {
        if (floor->getDownStairs().size() < index) {
    ...
    for (index = 0; index < floor->getMonsterCount(); index++) {
        x = floor->getMonsters().at(index)->getX();
        y = floor->getMonsters().at(index)->getY();
        classification = floor->getMonsters().at(index)->getClassification();
        speed = floor->getMonsters().at(index)->getSpeed();
        level = floor->getMonsters().at(index)->getLevel();
        lastSpottedX = floor->getMonsters().at(index)->getPlayerLastSpottedX();
        lastSpottedY = floor->getMonsters().at(index)->getPlayerLastSpottedY();
        // Write x Position
        if (error_check_fwrite(&(x), sizeof(x), 1, file)) {
            return 1;
        }
