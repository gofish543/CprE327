...
Dungeon::Dungeon(Settings* incomingSettings) {
    u_char index;

    this->settings = settings = incomingSettings;

    for(index = 0; index < DUNGEON_TEXT_LINES; index++) {
        this->textLines[index] = (char * ) calloc(sizeof(u_char) * (FLOOR_WIDTH + 1));
    }
}
...
Dungeon::~Dungeon() {
    ...
    for(index = 0; index < DUNGEON_TEXT_LINES; index++) {
        free(this->textLines[index]);
        this->textLines[index] = null;
    }
}
